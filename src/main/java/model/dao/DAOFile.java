...
public class DAOFile implements IDAO {
    ...
    public Person read(Person p) throws Exception {
        ...
        while (line != null) {
            ...
            if (data[1].equals(p.getNif())) {
                ...
                personToRead = new Person(data[0], data[1], date, data[3], data[4], data[5], photo);
                break;
            }
            line = br.readLine();
        }
    ...
    public ArrayList<Person> readAll() throws FileNotFoundException, IOException, ParseException {
        ...
        while (line != null) {
            ...
            people.add(new Person(data[0], data[1], date, data[3], data[4], data[5], photo));
            line = br.readLine();
        }
        br.close();
        return people;
    ...
    public void insert(Person p) throws IOException {
        ...
        if (p.getDateOfBirth() != null) {
            ...
            bw.write(p.getName() + "\t" + p.getNif() + "\t" + dateAsString + "\t" + p.getEmail() + "\t" + p.getPhoneNumber() + "\t" + p.getPostal()+ "\t");
        } else {
            bw.write(p.getName() + "\t" + p.getNif() + "\t" + "null" + "\t" + p.getEmail() + "\t" + p.getPhoneNumber() + "\t" + p.getPostal()+ "\t");
        }
        if (p.getPhoto() != null) {
